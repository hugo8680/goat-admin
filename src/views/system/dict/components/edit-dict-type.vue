      ...
      <a-row :gutter="16">
        <a-col :span="12">
          <a-form-item label="字典名称" field="dictName">
            <a-input v-model="state.form.dictName" placeholder="请输入字典名称"/>
          </a-form-item>
        </a-col>
        <a-col :span="12">
          <a-form-item label="字典类型" field="dictType">
            <a-input v-model="state.form.dictType" placeholder="请输入字典类型"/>
          </a-form-item>
        </a-col>
        <a-col :span="12">
...
const handleConfirm = () => {
  formRef.value?.validate().then((error: any) => {
    if (!error) {
      if (state.form.dictId !== undefined) {
        updateType(state.form).then(() => {
          Message.success("修改成功");
          emits('update:visible', false);
          emits('refreshDataList');
        });
      } else {
        addType(state.form).then(() => {
...
// 查询表单详情
const getDictTypeForm = async (id: number) => {
  const {data, code, msg} = await getType(id);
  if (code && code === 200) {
    state.form = data;
  } else {
    Message.error(msg)
